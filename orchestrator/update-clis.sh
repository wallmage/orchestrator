...
printf '%s\n' "$CLIS" | while IFS= read -r line; do
  ...
  s=$(date +%s); log "$cli: snapshot help/models"
  OLD=$("$cli" --version 2>&1 | head -1); snap before
  log "$cli: running '$upd' (live output below)"
  { sh -c "$upd" 2>&1; echo $? > "$T/rc"; } | tee "$T/$cli.upd" | sed 's/^/    | /'
  rc=$(cat "$T/rc"); [ "$rc" = 0 ] || echo "UPDATE FAILED (exit $rc): $(tail -3 "$T/$cli.upd" | tr '\n' ' ')"
  log "$cli: update finished, re-snapshot"
