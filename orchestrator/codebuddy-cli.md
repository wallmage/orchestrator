...
- Analysis-only: `--permission-mode plan` INSTEAD of `-y` — Read/Grep/Glob allowed, Write + Bash denied (no `git diff` via shell; point it at files). Reads are cwd-scoped and `--add-dir` does NOT widen them — prompt points outside the project (skill templates) → `WD=$HOME` (unverified).
