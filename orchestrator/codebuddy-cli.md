...
Flags:
- `--model <slug>` EVERY dispatch. Active: `kimi-k3-2` = debate seat 3 only. Dormant — no roster role, NEVER auto-triggered; the user promotes them by hand when Claude/Cursor quota is gone, or when the orchestrator has no Claude-side `Workflow` (e.g. a Codex/GPT orchestrator) and needs a workflow engine: `deepseek-v4.1-flash` (40 IQ / 74% SWE, fastest — first pick), `glm-5.3-flash` (42 / 63% at max — alternate). All others BANNED (`hy4-preview` dropped).
- Effort per model (verified in the CLI's model catalog): `deepseek-v4.1-flash` has NO effort mapping — omit `--effort` (accepted, no-op); `glm-5.3-flash` maps only `low|high|max` — `medium`/`xhigh` silently dropped; `kimi-k3-2` → `--effort max`. `--effort ultracode` is harness-side (any model, see below).
...
- Workflow model: `deepseek-v4.1-flash` (Workflow tool present in its tool list; 74% SWE, fastest) — `glm-5.3-flash` only if deepseek's scripts prove weak.
- `--effort ultracode` = harness flag: sets `workflowEffortLevel` + injects the plan-workflows reminder, independent of the model's effort map (works on deepseek); on glm it also means effort high. Keyword `ultracode` in prompt arms it per-message too. Same runner otherwise.
