...
Flags:
- `--model <slug>` EVERY dispatch. `kimi-k3-2` = debate seat 3 only. `deepseek-v4.1-flash` (40 IQ / 74% SWE) = dormant: no roster role, never auto-triggered; the user promotes it by hand when Claude/Cursor quota is gone or a non-Claude orchestrator needs a workflow engine. All others BANNED.
- `--effort`: `kimi-k3-2` → `max`; deepseek has no mapping — omit. `ultracode` accepted though absent from help (below).
- One slug per model, 300K/1M context tiers not selectable. `--autocompact 300k` keeps every request under 300K input — add if Tencent bills by request size (verify on the usage dashboard); cost = earlier compaction on long jobs.
...
Ultracode / Workflows (Claude Code's workflow engine; `ultracode` = model plans workflows unprompted, else the prompt must ask). ONLY on promoted deepseek, ONLY when Claude-side `Workflow` is unavailable (quota gone, Claude down, non-Claude orchestrator) or the fleet must outlive the session (CLI jobs keep writing `.final.txt`, re-adopted via bare `watcher.sh`); single jobs run plain:
