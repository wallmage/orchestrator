...
Flags:
- `--model deepseek-v4.1-flash` (40 IQ / 74% SWE) EVERY dispatch — dormant, never auto-triggered. All others BANNED.
...
- `CODEBUDDY_STREAM_KEEPALIVE_INTERVAL_MS=<ms>` (env, ≥2.154.0): stream-json emits `{"type":"system","subtype":"keepalive"}` per interval while a turn runs — event-loop bound, so it stops on a real hang: slow vs hung. Unset/non-numeric/≤0 = off (output byte-identical to before).
