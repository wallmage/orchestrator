...
| `judgment-reviewer.md` | Does the code actually work across files, state, errors, time? | once, whole branch after all merges | Different family than the author: grok-written → Workflow `model:'opus', effort:'high'`; Opus-written → Cursor CLI `cursor-grok-4.6-xhigh-fast --mode ask` |
